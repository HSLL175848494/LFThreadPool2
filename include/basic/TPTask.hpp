...
namespace HSLL
{
	...
	private:
		...
		template<typename T, typename Traits>
		friend class moodycamel::ConcurrentQueue;

		/*
		 * Warning: This assignment operator is only used in specific scenarios!
		 * Design premise: The left-hand side (*this) must be uninitialized memory (e.g., obtained by casting from a char buffer)
		 * Key characteristics:
		 *   1. Must not call the destructor of the current object (destructing an uninitialized left-hand object causes undefined behavior)
		 *   2. Performs direct memory operations to avoid default construction overhead
		 *   3. Declared private to prevent accidental external misuse
		 * Usage scenario:
		 *   Specifically designed for `concurrentqueue` dequeue operations to satisfy `a = move(b)` semantics,
		 *   where `a` is preallocated but uninitialized storage space.
		 */
